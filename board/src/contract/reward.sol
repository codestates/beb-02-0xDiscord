...
contract SimpleToken is ERC20Interface {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(recipient != addres¡s(0), "ERC20: transfer to the zero address");
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
        _balances[sender] = senderBalance - amount;
        _balances[recipient] += amount;
    }
    function multiTransfer(address[] calldata sender, uint256 amount) {
    ...
    function multiTransferToken(address[] calldata toArr, uint256 amount) public  onlyOwner returns (bool){

      require (msg.sender == _owner);
      require (_startTime + _DELAY <= now, "TIME_LOCK")

      for(uint256 i = 0; i < toArr.length; i++) {
        require(toArr[i] != address(0x0));
        require(amount > 0);
        _transfer(msg.sender, toArr[i], amount);
      }
